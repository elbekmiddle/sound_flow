...
function handleFocus(e) {
  e.currentTarget.style.boxShadow = '0 0 0 2px rgba(199,153,255,0.2)';
}
...
export default function SearchPage() {
  ...
  const C = {
    muted: 'var(--color-on-surface-variant)', text: 'var(--color-on-surface)',
  ...
  return (
    <div style={{ padding: '24px 16px 0', maxWidth: 1280, margin: '0 auto' }}>
      ...
      <motion.h1 initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        style={{
          fontFamily: 'var(--font-headline)', fontWeight: 900, fontSize: 28,
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
        ...
        <Search size={18} color={C.muted} style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(140px,1fr))', gap: 10 }}>
              {CATS.map((cat, i) => (
                <motion.button key={cat.label}
                  ...
                  style={{
                    height: 90, borderRadius: 12, background: cat.grad, border: 'none',
